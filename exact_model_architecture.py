#!/usr/bin/env python3
"""
EXACT Model Architecture - 100% Weight Compatible
Based on exact analysis of best_fast_high_accuracy_model.pth weights
Uses the exact same naming convention as the trained model
"""
...
class ExactModel(nn.Module):
    """EXACT weight compatible model architecture"""
    ...
    def __init__(self, num_chars=72, max_length=8, num_plate_types=9):
        ...
        # === EXACT BACKBONE STRUCTURE (using dot notation) ===
    ...
    def _get_custom_state_dict(self):
        """Get state dict with dot notation to match trained model"""
        state_dict = {}
        for name, param in self.named_parameters():
            # Convert underscores to dots
            custom_name = name.replace('_', '.')
            state_dict[custom_name] = param
        return state_dict

def load_exact_model(model_path="best_fast_high_accuracy_model.pth"):
    """Load the exact model with 100% weight compatibility"""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Create model with exact architecture
    model = ExactModel(num_chars=72, max_length=8, num_plate_types=9)
    model.to(device)

    # Load checkpoint
    checkpoint = torch.load(model_path, map_location=device)

    # Load with custom state dict
    try:
        model.load_state_dict(checkpoint, strict=True)
        logger.info("SUCCESS: 100% WEIGHT COMPATIBILITY ACHIEVED!")
        ...
        for k, v in checkpoint.items():
            # Convert checkpoint key to model key format
            model_key = k.replace('.', '_')
            if model_key in model_dict and v.shape == model_dict[model_key].shape:
                pretrained_dict[model_key] = v
            else:
                logger.warning(f"Skipping parameter: {k}, shape: {v.shape}")

        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict)
        logger.info(f"Partial load: {len(pretrained_dict)}/{len(checkpoint)} parameters loaded")
        return model

def test_exact_model():
    """Test the exact model architecture"""
    print("TESTING EXACT MODEL ARCHITECTURE")
